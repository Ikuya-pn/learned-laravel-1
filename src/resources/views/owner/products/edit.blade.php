                            ...
                            <div class="lg:w-1/2 md:w-2/3 mx-auto">
                                <form method="post" action="{{route('owner.products.update', ['product' => $product->id])}}">
                                    ...
                                    <div class="flex flex-wrap -m-2"> 
                                        ...
                                        <div class="p-2 w-full">
                                            <div class="relative">
                                                <label for="shop" class="leading-7 text-sm text-gray-600">販売するショップ</label>
                                                <div>
                                                    <select name="shop" id="shop" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                                        <option value="">販売するショップを選択してください</option>
                                        ...
                                        <div class="p-2 w-full">
                                            <div class="relative">
                                              ...
                                              <div class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-100 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-600 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                                {{$quantity}}
                                              </div>
                                            </div>
                                        </div>
                                        <div class="p-2 w-full">
                                            <div class="relative">
                                              <label for="quantity" class="leading-7 text-sm text-gray-600">在庫数の変更</label><span class="text-xs ml-2 text-gray-600">※0~500で指定してください。</span>
                                            ...
                                            <div class="relative flex justify-around mt-2">
                                                <div><input type="radio" name="type" checked value="1">増量する</div>
                                                <div><input type="radio" name="type" value="2">減量する</div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="relative">
                                            <x-select-image name="image1" :images="$images" currentId="{{$product->image1}}" currentImage="{{$product->firstImage->filename ?? ''}}" />
                                            <x-select-image name="image2" :images="$images" currentId="{{$product->image2}}" currentImage="{{$product->secondImage->filename ?? ''}}" />
                                            <x-select-image name="image3" :images="$images" currentId="{{$product->image3}}" currentImage="{{$product->thirdImage->filename ?? ''}}" />
                                            <x-select-image name="image4" :images="$images" currentId="{{$product->image4}}" currentImage="{{$product->fourthImage->filename ?? ''}}" />
                                            <x-select-image name="image5" :images="$images" currentId="{{$product->image1}}" />
                                        </div>
                                        <div class="p-2 w-full mt-3">
                                            <div class="relative flex justify-around">
                                              <div><input type="radio" name="is_selling" @if($product->is_selling === 1) checked @endif value="1">販売中</div>
                                              <div><input type="radio" name="is_selling" @if($product->is_selling === 0) checked @endif value="0">停止中</div>
                                            </div>
                                        </div>
                                        <div class="mt-4 p-2 w-full flex justify-between items-center">
                                            <a href="{{route('owner.products.index')}}" class="border-b border-gray-500 text-gray-600 hover:text-gray-400">戻る</a>
                                ...
                                <div class="mt-10 text-right">
                                    <form id="delete_{{$product->id}}" method="post" action="{{route('owner.products.destroy', ['product' => $product->id])}}">
                                        @method('delete')
                                        @csrf
                                        <a href="#" data-id="{{$product->id}}" onclick="deletePost(this)" class="border-b border-gray-500 hover:text-gray-300 hover:border-gray-300">削除する</a>
                                    </form>
                                </div>
                            </div>
                        </div>
